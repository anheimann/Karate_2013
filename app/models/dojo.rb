class Dojo < ActiveRecord::Base
  ...
  # method that creates a string of a basic, static Google map centered on the latitude and longitude of the dojo in question
  def self.create_map_link(dojos, zoom=12,width=400,height=400)
    return map = "" if dojos.empty?   # || !dojos.kind_of? Array
    markers = ""; i = 1
    dojos.each do |dojo|
      markers += "&markers=color:red%7Ccolor:red%7Clabel:#{i}%7C#{dojo.latitude},#{dojo.longitude}"
      i += 1
    end
    map = "http://maps.google.com/maps/api/staticmap?center= #{dojos.first.latitude},#{dojos.first.longitude}&zoom=#{zoom}&size=#{width}x#{height}&maptype=roadmap#{markers}&sensor=false"
  end
